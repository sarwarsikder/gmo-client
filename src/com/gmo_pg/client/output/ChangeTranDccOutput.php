...
/**
 * <b>多通貨クレジット決済（DCC)金額変更　出力パラメータクラス</b>
 *
 ...
 */
class ChangeTranDccOutput extends BaseOutput {
	...
	private $dccCurrency;
	/**
	 * @var bigDecimal 支払通貨決済額(金額変更後)
	 */
	private $dccAmount;
	/**
	 * @var bigDecimal 為替レート(金額変更後)
	 */
	private $dccRate;
	/**
	...
	/**
	 * 支払通貨決済額(金額変更後)取得
	 * @return bigDecimal 支払通貨決済額(金額変更後)
	 */
	...
	public function toString() {
		...
		$str .='&';
		$str .= 'ACSUrl=' . $this->encodeStr($this->getACSUrl());
		$str .='&';
		$str .= 'PaReq=' . $this->encodeStr($this->getPaReq());
		$str .='&';
